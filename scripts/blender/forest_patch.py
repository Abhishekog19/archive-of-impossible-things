...
# Separate review context replaces only local coarse tree proxies. Source world
# stays untouched, and collision proxies are removed only at replaced trunk roots.
...
for obj in list(bpy.context.scene.objects):
    ...
    if local and (plant_proxy or trunk_proxy):
...
report['removedContextProxies'] = removed
(OUT / 'forest-patch-report.json').write_text(json.dumps(report, indent=2) + '\n')
