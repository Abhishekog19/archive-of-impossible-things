...
def export_context(include_hub=None):
    ...
    removed = []
    hub_trees = {'Tree trunk.'+str(i).zfill(3) for i in (2,3,4)}
    hub_trees.update(prefix+'.'+str(i).zfill(3) for prefix in ('Tree limb','Forest crown') for i in range(10,25))
    for obj in list(bpy.context.scene.objects):
        ...
        hub_proxy = include_hub and (obj.name.startswith(('Plaza paving', 'Plaza ring', 'Column base', 'Broken column course')) or obj.name in hub_trees)
