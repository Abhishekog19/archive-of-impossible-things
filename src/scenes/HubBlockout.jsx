...
export default function HubBlockout({ reference = false, cornerView = false, view = 'reference', legacy = false, waterStudy = false }) {
  const { nodes } = useGLTF(legacy ? '/models/hub-blockout.glb' : '/models/world-blockout.glb')
  const camera = useThree((s) => s.camera)
  useEffect(() => {
    if (!reference) return
    const pose = waterStudy && view === 'cavern' ? CAVERN_WATER.camera : worldViews[view] || worldViews.reference
    camera.position.set(...(cornerView ? [-8, 5, 1] : pose.position))
    camera.lookAt(...(cornerView ? [-8, 1.9, -10] : pose.target))
    camera.updateProjectionMatrix()
  }, [camera, reference, cornerView, view, waterStudy])
